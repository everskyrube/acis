                         ...
                         double duration)
{
  ...
  if (end_x > x_max)
  {
    cout << " end_x is larger than x_max " << endl;
  ...
  if (end_x < x_min)
  {
    cout << " end_x smaller is than x_min " << endl;
  ...
  if (end_y > y_max)
  {
    cout << " end_y is larger than y_max " << endl;
  ...
  if (end_y < y_min)
  {
    cout << " end_y is smaller than y_min " << endl;
...
void GeneralMove::getPose(double time, geometry_msgs::PoseStamped &pose)
{
  ...
  if (est_flag == 0)
  {
    ...
    {
      double hv = (sqrt(pow(start_x - end_x, 2) + pow(start_y - end_y, 2))) / est_t;
      double vv = (sqrt(pow(start_z - end_z, 2))) / est_t;
      ...
      if (vv > vv_limit)
      {
        std::cout << "check vertical velocity limit " << std::endl;
      }
      if (hv > hv_limit)
      {
        std::cout << "check horizontal velocity limit " << std::endl;
      }
      if (av > av_limit)
      {
        std::cout << "check angular velocity limit " << std::endl;
      }
