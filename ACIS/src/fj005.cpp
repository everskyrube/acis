...
static double o_x, o_y, o_z, t1_x, t1_y, t1_z, t1_yaw, t2_x, t2_y, t2_z, t2_yaw, p1_x, p1_y, p1_z, p1_yaw, p2_x, p2_y, p2_z, p2_yaw;
...
static stage mission_stage = IDLE;
...
int main(int argc, char **argv)
{
  ...
  p1_x = getDoubleVariableFromYaml(config_file_path, "p1_x");
  p1_y = getDoubleVariableFromYaml(config_file_path, "p1_y");
  p1_z = getDoubleVariableFromYaml(config_file_path, "p1_z");
  p1_yaw = getDoubleVariableFromYaml(config_file_path, "p1_yaw");

  p2_x = getDoubleVariableFromYaml(config_file_path, "p2_x");
  p2_y = getDoubleVariableFromYaml(config_file_path, "p2_y");
  p2_z = getDoubleVariableFromYaml(config_file_path, "p2_z");
  p2_yaw = getDoubleVariableFromYaml(config_file_path, "p2_yaw");
  ...
  while (ros::ok())
  {
    ...
    if (mission_stage == TAKEOFF2)
    {
      ...
      identify_flag.identify = true;
      static GeneralMove takeoff2(ros::Time::now().toSec(), t1_x, t1_y, t1_z, 0, t2_x, t2_y, t2_z, t2_yaw, 6);
    ...
    if (mission_stage == HOVER1)
    {
      ...
      if (ros::Time::now() - last_request > ros::Duration(6.0))
      {
        ...
        cout << "mission stage : Hovering 1 --------------------------" << endl;
        last_request = ros::Time::now();
      }
    }
    ...
    {
      identify_flag.identify = true;
      static GeneralMove standby(ros::Time::now().toSec(), tmp_x, tmp_y, tmp_z, tmp_yaw, p1_x, p1_y, p1_z, p1_yaw, 10);
    ...
    {
      identify_flag.identify = true;
      static GeneralMove inspect(ros::Time::now().toSec(), p1_x, p1_y, p1_z, p1_yaw, p2_x, p2_y, p2_z, p2_yaw, 33);
      inspect.getPose(ros::Time::now().toSec(), pose);
      if (inspect.finished())
      {
        inspect.getEnding(tmp_x, tmp_y, tmp_z, tmp_yaw);
        mission_stage = HOVER3;
        cout << "mission stage : At Point2 --------------------------" << endl;
        last_request = ros::Time::now();
      }
    }
