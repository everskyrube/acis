...
void yoloNet::runOnFrame(Mat &frame)
{
  if (this->flag == 0)
  {
    ...
    Mat blob;
    // before create a 4D blob from a frame，scale down the frame，swap RB channel, adjust the size, and save as blob object

    blobFromImage(frame, blob, this->scale, Size(this->width, this->height), Scalar(0, 0, 0), this->swapRB);
    // set blob as the input of network

    net.setInput(blob);
    std::vector<Mat> netOuts;
    this->dnn_start = chrono::steady_clock::now();
    net.forward(netOuts, this->net_outputNames); // forward propagation, compute net_outputNames 中存储的层的输出,and save to netOuts vector
    ...
    for (auto &output : netOuts)
    {
      ...
      for (int j = 0; j < output.rows; j++) // search matched bounding box
      {
        auto objectnessPrediction = output.at<float>(j, 4);
        if (objectnessPrediction >= this->confThreshold) // confidence larger than threshold
        {
    ...
    for (int i = 0; i < indices.size(); i++)
    {
      ...
      */
      int idx = indices[i];
      /*          cout<<"idx: "<< idx<<endl;
                cout << "classIds[idx] " << classIds[idx] <<endl;*/
      // class[idx] should range from 0 to number of class - 1 eg. 0,1,2,3
...
{
  // cout << "init_box function start" << endl;
...
bool yoloNet::check_detected()
{
  if (this->flag == 0)
  {
    return (this->objects.size() == 0) ? false : true;
  }
}

void yoloNet::init_points()
{
  // cout << "init_points function" << endl;
  for (int i = 0; i < this->objects.size(); i++)
  {
    // dep_points.clear();
    // env_points.clear();
    ...
    // cout << "Depth_box: "<< "top left: " << objects[i].depth_box.tl() << ", bottom right: " << objects[i].depth_box.br()<< endl;
